...
namespace InvestQ.API.Controllers
{
    ...
    public class AdministradorDeFundoImobiliarioController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<IActionResult> GetAdministradorDeFundoImobiliarioById(Guid id)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(Guid id, AdministradorDeFundoImobiliarioDto model)
        {
            try
            {
                if (model.Id != id) 
